...
test('test', async ({ page }) => {
  ...
  await page.getByRole('row', { name: 'lahore branch one Acube' }).locator('button[name="delete"]').click();
  await page.getByRole('button', { name: 'OK' }).click();
  await page.getByRole('heading', { name: 'Branch Deleted Successfully.' }).click();
});
